...
def test_atr_stop_pct_uses_atr():
    df = _make_df([100 + i for i in range(50)])
    cfg = replace(auto_trader.CONFIG, atr_period=14, atr_mult=1.5, sl_pct=0.0)
    pct = auto_trader._atr_stop_pct(df, cfg)
    assert pct > 0
...
def test_calc_quote_amount_respects_risk_fraction(monkeypatch):
    ...
    settings = replace(auto_trader.CONFIG, quote_usdt=10_000.0)
    quote = auto_trader._calc_quote_amount(equity=5_000.0, sl_pct=0.01, cfg=cfg, settings=settings)
...
def test_risk_snapshot_from_positions(monkeypatch):
    ...
    settings = replace(auto_trader.CONFIG, use_portfolio_risk=True, max_portfolio_risk=0.0)
    ...
    snap = auto_trader._risk_snapshot_from_positions(positions, equity=10_000.0, settings=settings)
